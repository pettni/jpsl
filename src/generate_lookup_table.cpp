...
int main(int argc, char const *argv[]) {
  ...
  for (uint16_t iter=0; iter!=256; ++iter) {
    unordered_set<Dir> obstacles = decode_obs_1d(uint8_t(iter));
    unordered_set<Dir> forced = forced_neighbors_slow(Point(0,0,0), Point(-1,0,0), [obstacles] (const Point & p) {
  ...
  for (uint16_t iter=0; iter!=256; ++iter) {
    unordered_set<Dir> obstacles = decode_obs_2d(uint8_t(iter));
    unordered_set<Dir> forced = forced_neighbors_slow(Point(0,0,0), Point(-1,-1,0), [obstacles] (const Point & p) {
      return obstacles.find(Point(0,0,0).direction_to(p)) == obstacles.end();});
  ...
  for (uint16_t iter=0; iter!=64; ++iter) {
    unordered_set<Dir> obstacles = decode_obs_3d(uint8_t(iter));
    unordered_set<Dir> forced = forced_neighbors_slow(Point(0,0,0), Point(-1,-1,-1), [obstacles] (const Point & p) {
      return obstacles.find(Point(0,0,0).direction_to(p)) == obstacles.end();});
