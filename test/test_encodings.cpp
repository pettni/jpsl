...
TEST(test_encodings, test_encodings) {
  ...
  for (uint16_t prob=0; prob != (1<<7); ++prob) {
    unordered_set<Dir> dec = decode_obs_1d(prob);
  ...
  for (uint16_t prob=0; prob != (1<<7); ++prob) {
    unordered_set<Dir> dec = decode_obs_2d(prob);
  ...
  for (uint16_t prob=0; prob != (1<<5); ++prob) {
    unordered_set<Dir> dec = decode_obs_3d(prob);
  ...
  for (uint16_t prob=0; prob != (1<<7); ++prob) {
    unordered_set<Dir> dec = decode_fn_1d(Point(0,0,0), Point(-1,0,0), prob);
    EXPECT_EQ(prob, encode_fn_1d(dec));
  }

  for (uint16_t prob=0; prob != (1<<11); ++prob) {
    unordered_set<Dir> dec = decode_fn_2d(Point(0,0,0), Point(-1,0,0), prob);
    EXPECT_EQ(prob, encode_fn_2d(dec));
  }

  for (uint16_t prob=0; prob != (1<<5); ++prob) {
    unordered_set<Dir> dec = decode_fn_3d(Point(0,0,0), Point(-1,0,0), prob);
